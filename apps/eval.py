...
class Evaluator:
    def __init__(self, opt, projection_mode='orthogonal'):
        ...
        # create net
        netG = TitmouseNet(opt, projection_mode).to(device=cuda)
        print('Using Network: ', netG.name)

        if opt.load_netM_checkpoint_path:
            print(opt.load_netM_checkpoint_path)
            netG.load_state_dict(torch.load(opt.load_netM_checkpoint_path, map_location=cuda))
    ...
    def load_image(self, image_path, mask_path, pcd_path):
        # Name
...
if __name__ == '__main__':
    ...
    test_pcds = glob.glob(os.path.join(opt.test_folder_path, '*.ply'))

    print("num; ", len(test_masks))

    for image_path, mask_path, pcd_path in tqdm.tqdm(zip(test_images, test_masks, test_pcds)):
        try:
            print(image_path, mask_path, pcd_path)
            data = evaluator.load_image(image_path, mask_path, pcd_path)
