...
def render_prt_ortho(out_path, folder_name, subject_name, shs, rndr, rndr_uv, im_size, angl_step=4, n_light=1, pitch=[0]):
    ...
    mesh_path = os.path.join(folder_name, '*.obj')
    mesh_file = glob.glob(mesh_path)[0]
    ...
    if not os.path.exists(face_prt_file):
        ...
        return
    text_file = os.path.join(folder_name, 'material_0.jpeg')
...
if __name__ == '__main__':
    import glob
    shs = np.load('/media/mana/mana/code/unzip/titmouse_pifu/env_sh.npy')

    parser = argparse.ArgumentParser()
    parser.add_argument('-i', '--input', type=str, default='/media/mana/mana/dataset/THuman2.0_Release/0000')
    parser.add_argument('-o', '--out_dir', type=str, default='/media/mana/mana/dataset/Titmouse_pifu')
    ...
    dataset_dir = '/media/mana/mana/dataset/THuman2.0_Release'
    item_path = os.path.join(dataset_dir, '*')
    item_list = glob.glob(item_path)
    # for item in sorted(item_list[19]):
    #     print(item)
    item = '/media/mana/mana/dataset/THuman2.0_Release/0083'
    subject_name = item.split('/')[-1]
    render_prt_ortho(args.out_dir, item, subject_name, shs, rndr, rndr_uv, args.size, 1, 1, pitch=[0])
