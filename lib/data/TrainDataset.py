...
class TrainDataset(Dataset):
    ...
    def __init__(self, opt, phase='train'):
        ...
        # 三维数据
        self.PCD = os.path.join(self.root, 'PCD')
    ...
    # 三维数据
    def get_pcd(self, subject, view_id):
        pcd_path = os.path.join(self.PCD, subject, 'titmouse_%s_watertight_pred.ply' % subject)
        ...
        pcd_data = data_np[:, :3]
        pcd_data_rorate = self.rorate_points(pcd_data, view_id)
        return {
            'pcd': torch.tensor(pcd_data_rorate).float()
        }

    def rorate_points(self, pts, view_id):
    ...
    def get_samples(self, subject, view_ids):
        pts_path = os.path.join(self.SAMPLES, subject, 'sample.mat')
        try:
            pts_data = sio.loadmat(pts_path, verify_compressed_data_integrity=False)
        except ValueError as e:
            print('Value error occurred when loading ' + pts_path)
            raise ValueError(str(e))

        pts_adp_idp = np.int32(np.random.rand(5000//2) * len(pts_data['surface_points_inside']))
        pts_adp_idn = np.int32(np.random.rand(5000//2) * len(pts_data['surface_points_outside']))
        pts_uni_idp = np.int32(np.random.rand(5000//32) * len(pts_data['uniform_points_inside']))
        pts_uni_idn = np.int32(np.random.rand(5000//32) * len(pts_data['uniform_points_outside']))
        ...
        pts_rorate = self.rorate_points(pts, view_ids)
        pts_ov = pts_ov.astype(np.float32)

        return {
            'ids': (pts_adp_idp, pts_adp_idn, pts_uni_idp, pts_uni_idn),
            'samples_pts': torch.Tensor(pts_rorate).transpose(1, 0),
            'labels_pts': torch.Tensor(pts_ov).transpose(1, 0)
        }

    def get_view_id(self, num_views, yid=0, pid=0, random_sample=False):
    ...
    def get_render(self, subject, num_views, yid=0, pid=0, random_sample=False):
        ...
        if random_sample:
            view_ids = np.random.choice(self.yaw_list, num_views, replace=False)
        # view_ids = self.get_view_id(num_views, yid=0, pid=0, random_sample=False)
    ...
    def select_sampling_method(self, subject):
        ...
        sigmas[curvs <= 0.004] = 0.025
        sigmas[curvs > 0.004] = 0.01
        ...
        sample_points = surface_points + random_shifts
    ...
    def get_item(self, index):
        ...
        res = {
            'name': subject,
            'mesh_path': os.path.join(self.OBJ, subject + '_watertight.obj'),
            'sid': sid,
            'yid': yid,
            'pid': pid,
            'b_min': self.B_MIN,
        ...
        view_ids, render_data = self.get_render(subject, num_views=self.num_views, yid=yid, pid=pid,
                                        random_sample=self.opt.random_multiview)
        res.update(render_data)

        pcd = self.get_pcd(subject, view_ids[-1])

        res.update(pcd)

        sample_pts = self.get_samples(subject, view_ids[-1])
        res.update(sample_pts)

        # pifu sampling method
