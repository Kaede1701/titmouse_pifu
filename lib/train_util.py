...
def gen_mesh(opt, net, cuda, data, save_path, use_octree=True):
    ...
    pcd_tensor = data['pcd'].to(device=cuda).unsqueeze(0).permute(0, 2, 1)
...
def calc_error(opt, net, cuda, dataset, num_tests):
    ...
    with torch.no_grad():
        ...
        for idx in tqdm(range(num_tests)):
            ...
            image_tensor = data['img'].to(device=cuda)
            calib_tensor = data['calib'].to(device=cuda)
            sample_tensor = data['samples_pts'].to(device=cuda).unsqueeze(0)
            # print(data['pcd'].size(), data['img'].size())
            pcd_tensor = data['pcd'].to(device=cuda).unsqueeze(0).permute(0, 2, 1)
            if opt.num_views > 1:
                sample_tensor = reshape_sample_tensor(sample_tensor, opt.num_views)
            label_tensor = data['labels_pts'].to(device=cuda).unsqueeze(0)

            res, error = net.forward(image_tensor, pcd_tensor, sample_tensor, calib_tensor, labels=label_tensor)
